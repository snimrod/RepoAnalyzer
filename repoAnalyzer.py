...
def handlePage(owner, repo, pr, dtype, page):
    if (len(pr) > 0):
...
# api-endpoint 
#URL = "https://api.github.com/repos/{own}/{rep}/pulls/{pull}{type}?page={p}&per_page=100".format(own = owner, rep = repo, pull = pr, type = dtype, p = page)
...
i = 1
while (handlePage(owner, repo, pr, dtype, i) > 0):
    i = i + 1
